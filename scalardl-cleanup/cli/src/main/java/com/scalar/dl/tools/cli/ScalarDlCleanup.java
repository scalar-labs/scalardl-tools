...
/**
 * Top-level command for the ScalarDL cleanup tools. It groups the operator-facing subcommands into
 * a single binary.
 */
@Command(
...
public class ScalarDlCleanup implements Runnable {
  ...
  static CommandLine createCommandLine() {
    return new CommandLine(new ScalarDlCleanup())
        .setParameterExceptionHandler(
            (ex, args) -> {
              ...
              // USER_ERROR instead.
              Common.printError(new ScalarDlCleanupException(ex.getMessage(), Category.USER_ERROR));
