...
public final class CompletionToken {
   ...
   * @throws ScalarDlCleanupException if the string cannot be decoded or the CRC32C does not match
   */
    ...
    try {
      ...
      if (!expected.equals(crc)) {
        throw new ScalarDlCleanupException(
            ScalarDlCleanupError.COMPLETION_TOKEN_CRC_MISMATCH, expected, crc);
      }
      return new CompletionToken(serverType, startedAtMs, crc);
    } catch (ScalarDlCleanupException e) {
      throw e;
    } catch (Exception e) {
      throw new ScalarDlCleanupException(ScalarDlCleanupError.COMPLETION_TOKEN_DECODE_FAILED, e);
    }
  }
  ...
  public enum ServerType {
    ...
    static ServerType fromValue(String value) {
      ...
      throw new ScalarDlCleanupException(ScalarDlCleanupError.UNKNOWN_SERVER_TYPE, value);
    }
